...
const makeBookingCurrentTimes = (bikeId, selectDate) => {
    const currentBookings = bookings.filter(({ bike, date }) => bike.id === bikeId && selectDate === date);
...
const resolvers = {
    Query: {
        ...
        bookingTimes: (parent, args) => {
            const bookingTime = makeInitialBookingTime();
            const times = makeBookingCurrentTimes(args.bikeId, args.date);
            return bookingTime.map((ele) => times.includes(ele.time) ? { ...ele, available: false } : ele);
        },
    },
    Mutation: {
        book: (parent, args) => {
